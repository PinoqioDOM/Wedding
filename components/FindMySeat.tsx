...
export default function FindMySeat({ tables, seats, guests }: Props) {
  ...
  return (
    <div className="card p-6 md:p-10">
      <input
        className="input"
        placeholder="დაიწყე სახელის აკრეფა…"
        value={q}
      ...
      {result && result.found && (
        <div className="mt-8 text-center">
          <p className="label">თქვენი ადგილია</p>
          <p className="font-display text-5xl mt-2">{result.table?.label}</p>
          <p className="font-script text-gold-500 text-3xl mt-2">ადგილი №{result.seat.seat_index}</p>
          <p className="mt-6 text-ink-700/70">
            შევხვდებით საცეკვაო მოედანზე.
          </p>
        </div>
      )}
      ...
      {result && !result.found && (
        <div className="mt-8 text-center text-ink-700/70">
          ადგილი ჯერ არ გაქვთ მინიჭებული — გთხოვთ, დაუკავშირდეთ მასპინძელს.
        </div>
      )}
    </div>
  );
